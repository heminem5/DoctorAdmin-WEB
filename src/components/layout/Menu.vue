<template>
  <div
    class="menu"
    :style="{ width: `${!isCollapse ? 200 : 64}px` }"
    style="height: calc(100vh - 60px); transition: width 0.3s ease-in-out"
  >
    <div style="height: 100%; overflow-y: auto; overflow-x: hidden">
      ...
      >
        <template v-for="(menu, index) in menuList">
          <!--无子菜单-->
          <el-menu-item
            v-if="!menu.children && menu.id !== 'chat'"
            :key="menu.id"
            :index="menu.id + ''"
            @click="onRedirect(menu)"
          >
          ...
          <!--无子菜单 chat-->
          <el-menu-item
            v-if="menu.id === 'chat' && conf.EnableChat"
            :key="menu.id"
            :index="menu.id + ''"
            @click="onRedirect(menu)"
          >
          ...
          <el-sub-menu v-if="menu.children" :key="menu.id" :index="menu.id">
            ...
            <template v-for="child in menu.children" :key="child.id">
              <el-menu-item
                v-if="
                  user.id && child.roles.indexOf(user.role.toLowerCase()) !== -1
                "
