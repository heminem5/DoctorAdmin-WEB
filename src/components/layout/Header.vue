      ...
      <div class="x-row-start">
        <div class="x-pointer">
          <el-tooltip effect="light" content="用户信息" placement="bottom-start">
            <img :src="`${ publicPath}images/user.png`" @click="personal" style="height: 20px">
          </el-tooltip>
        </div>
        <div style="font-size: 14px;margin-left: 5px;margin-right: 8px">{{ initGreeting() }}！{{ userInfo.username }} 【{{ userInfo.role }}】</div>
      </div>
...
import doctorPersonal from '../../views/doctor/Edit'
import patientPersonal from '../../views/patient/Edit'
const personal = async ()=>{
  ...
  if(userInfo.value.role === 'doctor'){
    const op = Dialog.open(doctorPersonal, `个人中心`)
    op.mounted(c => {
      c.render(id,()=>{Msg.success("个人信息已更新")})
    })
    op.confirm(async (c) => {
  ...
  if(userInfo.value.role === 'patient'){
    const op = Dialog.open(patientPersonal, `个人中心`)
    op.mounted(c => {
      c.render(id,()=>{Msg.success("个人信息已更新")})
    })
    op.confirm(async (c) => {
