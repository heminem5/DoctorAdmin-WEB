    ...
    <x-main>
        ...
        <el-row class="search-row">
                            ...
                            <div style="width: 222px">
                                <el-date-picker placeholder="测试时间起始" value-format="YYYY-MM-DD HH:mm:ss" v-model="searchForm.createtimeL" type="datetime" :shortcuts="shortcuts"/>
                            </div>
                            <span>至</span>
                            <div style="width: 222px">
                                <el-date-picker placeholder="测试时间结束" value-format="YYYY-MM-DD HH:mm:ss" v-model="searchForm.createtimeR" type="datetime" :shortcuts="shortcuts"/>
                            </div>
                        </div>
                            ...
                            <span>总分</span>
                                                            <div style="width: 150px">
                                    <el-input clearable placeholder="总分起始值" v-model.number="searchForm.scoreL" type="number"/>
                                </div>
                                <span>至</span>
                                <div style="width: 150px">
                                    <el-input clearable placeholder="总分结束值" v-model.number="searchForm.scoreR" type="number"/>
                                </div>
                                                    </div>
                                                                                                            <div class="search-item">
                            <span>结果</span>
                                                            <div style="width: 150px">
                                    <el-input clearable placeholder="请输入结果" v-model="searchForm.jieg" maxlength="255"/>
                                </div>
                                                    </div>
                                                
            <button class="btn btn-m" type="primary" @click="loadThisPage">查询</button>
            <button class="btn btn-success btn-m" v-if="user && ( user.role.toLowerCase() == 'patient'   )" @click="onAdd">新增</button>
        ...
        <el-table :data="pager.records" style="width: 100%">
            ...
            <template #default="scope">
                <el-button v-if="scope.row.cesrFrn" type="primary" text bg @click="cesrDetail(scope.row.cesr)">{{ scope.row.cesrFrn.name }}</el-button>
            </template>
        </el-table-column>
                 <el-table-column label="观察到的抑郁">
            <template #default="scope">
                <span >{{scope.row.s1}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="抑郁主诉">
            <template #default="scope">
                <span >{{scope.row.s2}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="内心紧张">
            <template #default="scope">
                <span >{{scope.row.s3}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="睡眠减少">
            <template #default="scope">
                <span >{{scope.row.s4}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="食欲减退">
            <template #default="scope">
                <span >{{scope.row.s5}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="注意集中困难">
            <template #default="scope">
                <span >{{scope.row.s6}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="懒散">
            <template #default="scope">
                <span >{{scope.row.s7}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="感受不能">
            <template #default="scope">
                <span >{{scope.row.s8}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="悲观思想">
            <template #default="scope">
                <span >{{scope.row.s9}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="自杀观念">
            <template #default="scope">
            ...
            <el-table-column fixed="right" label="操作">
                <template #default="scope">
                    ...
                    <button class="btn btn-error" v-if="  user.role.toLowerCase() == 'patient' ||    user.role.toLowerCase() == 'doctor'  " @click="onDelete(scope.row.id)">删除</button>
...
<script setup name="madrs">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/madrs/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    //onExcel
    const onExcel = async () => {
        Http.download({},false,"/madrs/excel")
    }
    //删除
    const onDelete = async (id) => {
        const op = Msg.confirm('确定删除本条数据？')
        op.then(async () => {
            let {success, message} = await Http.post(`/madrs/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
    ...
    const importExcel = ()=>{
        const op = Dialog.open(ImportExcel, `导入MADRS量表数据`).setCancelText('').setConfirmText('')
        op.mounted(c => {
            c.render('madrs',loadThisPage)
        })
    }
</script>
