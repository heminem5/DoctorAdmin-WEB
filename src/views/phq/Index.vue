        ...
        <el-table :data="pager.records" style="width: 100%">
            ...
            <template #default="scope">
                <el-button v-if="scope.row.cesrFrn" type="primary" text bg @click="cesrDetail(scope.row.cesr)">{{ scope.row.cesrFrn.name }}</el-button>
                 ...
                 <el-table-column label="做事提不起劲没兴趣">
            <template #default="scope">
                <span >{{scope.row.s1}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="感到心情低落沮丧">
            <template #default="scope">
                <span >{{scope.row.s2}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="入睡难睡不安稳">
            <template #default="scope">
                <span >{{scope.row.s3}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="感觉疲倦没活力">
            <template #default="scope">
                <span >{{scope.row.s4}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="食欲不振或吃太多">
            <template #default="scope">
                <span >{{scope.row.s5}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="觉得自己很糟">
            <template #default="scope">
                <span >{{scope.row.s6}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="对事物专注有困难">
            <template #default="scope">
                <span >{{scope.row.s7}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="动作或说话速度缓">
            <template #default="scope">
                <span >{{scope.row.s8}}</span>
            </template>
        </el-table-column>
                 <el-table-column label="有死或伤害自己的念头">
            <template #default="scope">
...
<script setup name="phq">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/phq/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    //onExcel
    const onExcel = async () => {
        Http.download({},false,"/phq/excel")
    }
    //删除
    const onDelete = async (id) => {
        const op = Msg.confirm('确定删除本条数据？')
        op.then(async () => {
            let {success, message} = await Http.post(`/phq/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
    ...
    const importExcel = ()=>{
        const op = Dialog.open(ImportExcel, `导入PHQ量表数据`).setCancelText('').setConfirmText('')
        op.mounted(c => {
            c.render('phq',loadThisPage)
        })
    }
</script>
