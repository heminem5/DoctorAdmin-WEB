...
<script setup name="doctor">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/doctor/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    //onExcel
    const onExcel = async () => {
        Http.download({},false,"/doctor/excel")
    }
    //删除
    const onDelete = async (id) => {
        const op = Msg.confirm('确定删除本条数据？')
        op.then(async () => {
            let {success, message} = await Http.post(`/doctor/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
    ...
    const importExcel = ()=>{
        const op = Dialog.open(ImportExcel, `导入医生数据`).setCancelText('').setConfirmText('')
        op.mounted(c => {
            c.render('doctor',loadThisPage)
        })
    }
</script>
