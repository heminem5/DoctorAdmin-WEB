<template>
  <x-main>
    ...
    <el-table :data="pager.records" style="width: 100%">
      ...
      <el-table-column label="病情">
        <template #default="scope">
          <span>{{ scope.row.bingq }}</span>
        </template>
      </el-table-column>
...
const loadThisPage = async () => {
  ...
  let { data, message } = await Http.get(`/patient/list`, params);
  console.log("查询条件");
  console.log(params);
  console.log("查询结果");
  console.log(data);
...
//onExcel
const onExcel = async () => {
  Http.download({}, false, "/patient/excel");
};
//删除
const onDelete = async (id) => {
  const op = Msg.confirm("确定删除本条数据？");
  op.then(async () => {
    let { success, message } = await Http.post(`/patient/delete?id=` + id);
    if (!success) {
      Msg.error(message);
    } else {
      Msg.success(message);
...
const importExcel = () => {
  const op = Dialog.open(ImportExcel, `导入患者数据`)
    .setCancelText("")
    .setConfirmText("");
  op.mounted((c) => {
    c.render("patient", loadThisPage);
  });
};
</script>
