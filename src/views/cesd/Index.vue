        ...
        <el-table :data="pager.records" style="width: 100%">
            ...
            <el-table-column fixed="right" label="操作">
                <template #default="scope">
                    ...
                    <button class="btn btn-error" v-if="  user.role.toLowerCase() == 'doctor' ||    user.role.toLowerCase() == 'patient'  " @click="onDelete(scope.row.id)">删除</button>
...
<script setup name="cesd">
    ...
    const importExcel = ()=>{
        const op = Dialog.open(ImportExcel, `导入CES-D量表数据`).setCancelText('').setConfirmText('')
        op.mounted(c => {
